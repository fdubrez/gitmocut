...
}.gitmojis.map(x => ({ ...x, keywords: [x.name] }));

let inputEl;
let emojiListEl;

const liItem = (x) => `<li class="list-group-item">${x.keywords[0]} ${x.emoji}</li>`;

window.addEventListener("DOMContentLoaded", () => {
  inputEl = document.querySelector("#search");
  emojiListEl = document.querySelector("#emojis");
  ...
  // listen submit form event (ENTER) to add emoji in clipboard & exit the app
  document.querySelector("#search-emoji-form").addEventListener("submit", async (e) => {
    e.preventDefault();
